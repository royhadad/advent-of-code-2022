...
// calculate result
let numberOfPairsWhereOneElfRangePartiallyOverlapsTheOther = 0;

elfPairs.forEach((elfPair) => {
  const { elf1, elf2 } = elfPair;

  if (elf1.start >= elf2.start && elf1.start <= elf2.end) {
    numberOfPairsWhereOneElfRangePartiallyOverlapsTheOther++;
  } else if (elf1.end >= elf2.start && elf1.end <= elf2.end) {
    numberOfPairsWhereOneElfRangePartiallyOverlapsTheOther++;
  } else if (elf2.start >= elf1.start && elf2.start <= elf1.end) {
    numberOfPairsWhereOneElfRangePartiallyOverlapsTheOther++;
  } else if (elf2.end >= elf1.start && elf2.end <= elf1.end) {
    numberOfPairsWhereOneElfRangePartiallyOverlapsTheOther++;
  }
});

const res = numberOfPairsWhereOneElfRangePartiallyOverlapsTheOther;
